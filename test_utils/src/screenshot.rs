...
/// - Difference image cannot be saved
...
fn pixel_difference(p1: Rgba<u8>, p2: Rgba<u8>) -> f32 {
    let r_diff = (f32::from(p1[0]) - f32::from(p2[0])).abs();
    let g_diff = (f32::from(p1[1]) - f32::from(p2[1])).abs();
    let b_diff = (f32::from(p1[2]) - f32::from(p2[2])).abs();
    let a_diff = (f32::from(p1[3]) - f32::from(p2[3])).abs();
...
impl GoldenFrameTest {
    ...
    /// 
    /// Returns an error if:
    /// - Golden frame directory cannot be created
    /// - Images cannot be saved or loaded
    /// - Comparison fails
...
mod tests {
    ...
    fn test_compare_identical_images() {
        const TOLERANCE: f32 = 0.000_001; // Very small tolerance for floating point comparison
        ...
        assert!((result.difference_percentage - 0.0).abs() < TOLERANCE);
    }
