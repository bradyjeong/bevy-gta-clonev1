...
impl EntityLimitManager {
    pub fn new() -> Self {
        ...
        limits.insert(EntityType::Building, EntityLimit { max_count: 200, spawn_rate: 0.08 });
        limits.insert(EntityType::Vehicle, EntityLimit { max_count: 50, spawn_rate: 0.04 });
        limits.insert(EntityType::NPC, EntityLimit { max_count: 20, spawn_rate: 0.01 });
        limits.insert(EntityType::Tree, EntityLimit { max_count: 100, spawn_rate: 0.05 });
    ...
    pub fn can_spawn(&self, entity_type: EntityType) -> bool {
        ...
        let limit = self.limits.get(&entity_type).map(|l| l.max_count).unwrap_or(0);
        *current < limit
    }
    ...
    pub fn get_spawn_rate(&self, entity_type: EntityType) -> f32 {
        self.limits.get(&entity_type).map(|l| l.spawn_rate).unwrap_or(0.0)
    }
}
