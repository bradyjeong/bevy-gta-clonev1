...
pub struct RoadNetwork {
    ...
    pub nodes: HashMap<u8, Vec3>, // Store node positions for grid-based road generation
...
impl RoadNetwork {
    /// Check if a position is near any road
    pub fn is_near_road(&self, position: Vec3, tolerance: f32) -> bool {
        // Debug: log how many roads we're checking
        if self.roads.is_empty() {
            println!("🚨 RoadNetwork.is_near_road: No roads in network!");
        }
        
        for road in self.roads.values() {
            // Check distance to road spline
            for i in 0..10 {
                let t = i as f32 / 9.0;
                let road_point = road.evaluate(t);
                if position.distance(road_point) <= tolerance {
                    return true;
                }
            }
        }
    ...
    /// Get the nearest point on any road
    ...
    /// Add a road node (for compatibility with layered_generation)
    pub fn add_node(&mut self, x: f32, z: f32) -> u8 {
        // Store node position for later connection
    ...
    /// Connect two nodes (for compatibility with layered_generation)
    pub fn connect_nodes(&mut self, from: u8, to: u8) {
        // Look up the stored node positions and create a road between them
        let (start, end) = match (self.nodes.get(&from), self.nodes.get(&to)) {
            (Some(a), Some(b)) => (*a, *b),
            _ => {
                println!("🚨 connect_nodes: Invalid node IDs {} -> {}", from, to);
                return;
            }
        };
        
        // Create an actual road segment between the nodes
        self.add_road(start, end, RoadType::SideStreet);
    }
    
    /// Find a path between two positions (placeholder for now)
    pub fn find_path(&self, _start: Vec3, _end: Vec3) -> Option<Vec<Vec3>> {
