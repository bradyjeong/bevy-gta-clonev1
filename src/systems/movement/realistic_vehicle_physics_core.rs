...
/// Core realistic vehicle physics system - focused on physics calculations only
pub fn realistic_vehicle_physics_core_system(
    time: Res<Time>,
    ...
    for (distance, _entity, mut velocity, mut transform, mut vehicle, mut dynamics, mut engine, mut suspension, mut tire_physics) in vehicles_with_distance {
        ...
        // STEP 1: Calculate engine forces with realistic power delivery
        let engine_force = calculate_engine_force(&mut engine, &dynamics, dt);
        
        // STEP 2: Calculate suspension forces (simplified without ground detection for now)
        let suspension_forces = Vec3::new(0.0, -dynamics.total_mass * 9.81, 0.0); // Basic gravity
        
        // STEP 3: Calculate tire forces with realistic friction model
        ...
        // STEP 5: Calculate weight transfer effects
