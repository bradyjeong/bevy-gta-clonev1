use crate::components::{ActiveEntity, Cullable, CullingSettings};
...
pub fn distance_culling_system(
    mut cullable_query: Query<
        (Entity, &mut Cullable, &mut Visibility, &Transform),
    ...
    let Ok((active_entity, active_transform)) = active_query.single() else {
        return;
    };
