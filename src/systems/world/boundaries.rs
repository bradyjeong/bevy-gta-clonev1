...
pub fn world_boundary_system(
    ...
    mut vehicle_query: Query<(&mut Velocity, &Transform), Or<(With<Car>, With<Helicopter>, With<F16>, With<Yacht>)>>,
) {
    for (mut velocity, transform) in vehicle_query.iter_mut() {
        ...
        if !bounds.contains(position_2d) {
            ...
            let clamped_x = position_2d.x.clamp(bounds.min.x + 10.0, bounds.max.x - 10.0);
            let clamped_z = position_2d.y.clamp(bounds.min.y + 10.0, bounds.max.y - 10.0);
            
            warn!("Vehicle exceeded world bounds - teleporting back from {:?} to ({:.1}, {:.1})", 
