use crate::components::{ActiveEntity, Car, F16, Helicopter, Player};
        ...
        match **_state {
            GameState::Walking => {
                if let Ok(player_transform) = _player_query.single() {
                    ...
                    {
                        info!(
                            "DEBUG: Player walking at position: {:?}",
            ...
            GameState::Driving => {
                if let Ok(car_transform) = _car_query.single() {
                    ...
                    if car_transform.translation.x.abs() > 100.0
                        || car_transform.translation.z.abs() > 100.0
                    {
                        info!(
                            "DEBUG: Car driving at position: {:?}",
            ...
            GameState::Flying => {
                if let Ok(helicopter_transform) = _helicopter_query.single() {
                    ...
                    if helicopter_transform.translation.y > 5.0
                        || helicopter_transform.translation.x.abs() > 100.0
                        || helicopter_transform.translation.z.abs() > 100.0
                    {
                        info!(
                            "DEBUG: Helicopter flying at position: {:?} (altitude: {:.1}m)",
                            helicopter_transform.translation, helicopter_transform.translation.y
                        );
                    }
                }
            }
            GameState::Jetting => {
                if let Ok(f16_transform) = _f16_query.single() {
                    ...
                    if f16_transform.translation.y > 10.0
                        || f16_transform.translation.x.abs() > 100.0
                        || f16_transform.translation.z.abs() > 100.0
                    {
                        info!(
                            "DEBUG: F16 flying at position: {:?} (altitude: {:.1}m)",
                            f16_transform.translation, f16_transform.translation.y
                        );
                    }
                }
            }
