...
use crate::components::world::{
    WorldBounds, BoundaryEffects, BoundaryZone, BoundaryVehicleType
};
use crate::components::vehicles::{VehicleState, VehicleHealth};
...
pub fn boundary_effects_system(
    ...
    mut vehicle_query: Query<(Entity, &mut Transform, &mut VehicleState, &mut BoundaryEffects, Option<&mut VehicleHealth>), With<ActiveEntity>>,
    time: Res<Time>,
) {
    for (entity, mut transform, mut vehicle_state, mut boundary_effects, vehicle_health) in &mut vehicle_query {
        ...
        match vehicle_type {
            BoundaryVehicleType::Aircraft => {
                apply_aircraft_boundary_effects(
                    entity, &mut commands, &mut transform, &mut vehicle_state, 
                    &boundary_effects, vehicle_health, &time
                );
            },
        ...
        if matches!(boundary_zone, BoundaryZone::OutOfBounds) {
            ...
            info!("Emergency teleport: {} moved from {:?} to safe zone {:?}", 
                  get_vehicle_type_name(vehicle_type), position, safe_pos);
        }
    }
}
...
fn get_boundary_vehicle_type(vehicle_state: &VehicleState) -> BoundaryVehicleType {
    ...
    match vehicle_state.vehicle_type {
        ...
        crate::components::vehicles::VehicleType::BasicCar => BoundaryVehicleType::GroundVehicle,
