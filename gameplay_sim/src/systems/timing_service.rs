...
//! Purpose:  Unified timing service that manages throttling intervals across all systems
//! Schedule: Update (continuous) and FixedUpdate (cleanup)
//! Reads:    Time, entity queries for cleanup
//! Writes:   TimingService resource, entity timer tracking
//! Invariants:
//!   * All system intervals are positive values
//!   * Entity timers are cleaned up when entities despawn
//!   * Global timing is updated exactly once per frame
