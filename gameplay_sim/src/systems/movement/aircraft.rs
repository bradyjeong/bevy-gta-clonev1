...
use game_core::components::{Helicopter, F16, ActiveEntity, MainRotor, TailRotor, AircraftFlight};
...
pub fn f16_movement(
    ...
    if flight.airspeed < flight.stall_speed && flight.current_thrust < 20.0 {
        ...
        let turbulence = Vec3::new(
            (time.elapsed_secs() * 3.0_f32).sin() * 2.0,
            (time.elapsed_secs() * 2.0_f32).cos() * 1.5,
