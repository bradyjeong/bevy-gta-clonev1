...
pub struct PerformanceConfig {
    ...
    pub weather_debug_interval: f32,  // 5.0 - Weather debug interval
...
pub struct AudioConfig {
    ...
    pub weather_volume: f32,      // 0.6 - Weather volume
...
pub struct WeatherConfig {
    // Weather transition
    pub transition_duration: f32,     // 5.0 - Weather transition time
    pub weather_change_probability: f32, // 0.0001 - Per-frame weather change chance
    
    // Visual effects
    pub rain_particle_density: f32,  // 300.0 - Rain particle density
    pub fog_particle_density: f32,   // 100.0 - Fog particle density
    pub particle_area_size: f32,     // 60.0 - Particle spawn area size
    pub particle_height: f32,        // 20.0 - Particle spawn height
    pub particle_lifetime: f32,      // 5.0 - Particle lifetime
    
    // Wind effects
    pub wind_strength: f32,          // 0.1 - Base wind strength
    pub wind_variation: f32,         // 0.1 - Wind direction variation
    
    // Physics effects
    pub rain_rates: WeatherRainRates,
    pub friction_modifiers: WeatherFrictionModifiers,
}

#[derive(Debug, Clone)]
pub struct WeatherRainRates {
    pub light_rain: f32,         // 0.1 - Light rain wetness rate
    pub heavy_rain: f32,         // 0.3 - Heavy rain wetness rate
    pub storm: f32,              // 0.5 - Storm wetness rate
}

#[derive(Debug, Clone)]
pub struct WeatherFrictionModifiers {
    pub wet_road_multiplier: f32,    // 0.8 - Wet road friction multiplier
    pub normal_friction: f32,        // 0.3 - Normal surface friction
}
...
impl Default for WorldConfig {
    fn default() -> Self {
        Self {
            ...
            lod_distances: [300.0, 600.0, 1000.0],
...
impl Default for WeatherConfig {
    fn default() -> Self {
        Self {
            ...
            weather_change_probability: 0.0001,
            rain_particle_density: 300.0,
            fog_particle_density: 100.0,
            particle_area_size: 60.0,
            particle_height: 20.0,
            particle_lifetime: 5.0,
            wind_strength: 0.1,
            wind_variation: 0.1,
            rain_rates: WeatherRainRates::default(),
            friction_modifiers: WeatherFrictionModifiers::default(),
        }
    }
}

impl Default for WeatherRainRates {
    fn default() -> Self {
        Self {
            light_rain: 0.1,
            heavy_rain: 0.3,
...
impl Default for WeatherFrictionModifiers {
    fn default() -> Self {
        Self {
            wet_road_multiplier: 0.8,
            normal_friction: 0.3,
        }
    }
}
...
impl GameConfig {
    ...
    pub fn validate_and_clamp(&mut self) {
        ...
        self.weather.validate_and_clamp();
...
impl PerformanceConfig {
    pub fn validate_and_clamp(&mut self) {
        ...
        self.weather_debug_interval = self.weather_debug_interval.clamp(1.0, 60.0);
...
impl AudioConfig {
    pub fn validate_and_clamp(&mut self) {
        ...
        self.engine_volume = self.engine_volume.clamp(0.0, 2.0);
        self.weather_volume = self.weather_volume.clamp(0.0, 2.0);
...
impl WeatherConfig {
    pub fn validate_and_clamp(&mut self) {
        // Clamp transition parameters
        self.transition_duration = self.transition_duration.clamp(1.0, 30.0);
        self.weather_change_probability = self.weather_change_probability.clamp(0.00001, 0.01);
        
        // Clamp visual effect parameters
        self.rain_particle_density = self.rain_particle_density.clamp(50.0, 1000.0);
        self.fog_particle_density = self.fog_particle_density.clamp(20.0, 500.0);
        self.particle_area_size = self.particle_area_size.clamp(20.0, 200.0);
        self.particle_height = self.particle_height.clamp(5.0, 100.0);
        self.particle_lifetime = self.particle_lifetime.clamp(1.0, 20.0);
        
        // Clamp wind parameters
        self.wind_strength = self.wind_strength.clamp(0.01, 1.0);
        self.wind_variation = self.wind_variation.clamp(0.0, 0.5);
        
        self.rain_rates.validate_and_clamp();
        self.friction_modifiers.validate_and_clamp();
    }
}

impl WeatherRainRates {
    pub fn validate_and_clamp(&mut self) {
        self.light_rain = self.light_rain.clamp(0.01, 0.5);
        self.heavy_rain = self.heavy_rain.clamp(0.1, 1.0);
        self.storm = self.storm.clamp(0.2, 2.0);
    }
}

impl WeatherFrictionModifiers {
    pub fn validate_and_clamp(&mut self) {
        self.wet_road_multiplier = self.wet_road_multiplier.clamp(0.2, 1.0);
        self.normal_friction = self.normal_friction.clamp(0.1, 2.0);
    }
}
