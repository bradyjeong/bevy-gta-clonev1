...
pub const LOD_DISTANCES: [f32; 3] = [300.0, 600.0, 1000.0];
...
impl PlacementGrid {
    ...
    pub fn can_place(&self, position: Vec3, content_type: ContentType, radius: f32, min_distance: f32) -> bool {
        let cell = self.world_to_grid(position);
        ...
        for dx in -1..=1 {
            for dz in -1..=1 {
                ...
                if let Some(entities) = self.grid.get(&check_cell) {
                    for (existing_pos, existing_type, existing_radius) in entities {
...
pub fn unified_world_streaming_system(
    ...
    let chunk_changed = world_manager.active_chunk != Some(current_chunk);
